...
Drop audiobook folders or groups of audio files into the book tree. Each folder is one book, and files selected in one action form one book. Expand or collapse books, drag chapters within a book for manual order, double-click chapter titles to edit them, select books to edit their metadata, choose whether chapter titles come from embedded tags or source file names, choose per-book quality/channel settings, choose one batch destination folder, and export all books. Each book shows its own export progress beside the title while the batch progress remains in the footer. Ctrl-click and Shift-click select multiple rows; right-click a selection to delete it. Projects can be saved as JSON from the **Project** menu.
...
Batch output uses the editable **Structure** template beside the destination folder. The default is `Destination\Author\[Series]\[Series Number - ]Title\Title.m4b`, but you can choose a flatter layout or enter a custom template. Supported fields are `{author}`, `{series}`, `{series_number}`, `{book}`, `{title}`, `{narrator}`, `{year}`, and `{source}`; separate folders with `/` or `\`, and let the final component be the M4B filename. Existing folders are reused. Books are exported sequentially and committed after validation; cancelling or failing a later book preserves all earlier completed books and stops the remaining queue.
...
Select a book and choose **Search metadata…** in the book-details panel to open the search form and start searching Google Books, Open Library, and the Library of Congress catalog immediately by title, author, or ISBN. Title/author searches retry with broader combined terms and inspect Open Library editions, so a series title plus subtitle can still match. ISBN-10/ISBN-13 variants are tried automatically, including a direct Open Library edition lookup and a Google Books legacy-feed fallback when the JSON endpoint is throttled. Edit the fields and search again if needed. Matching results are validated against the entered title and author before they are shown. Applying a result fills the available bibliographic fields and downloads its cover next to the source audio without replacing an existing file. Narrator and audiobook runtime are not supplied by these general book catalogs and remain available for manual entry.
